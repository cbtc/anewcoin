...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSeaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSeaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeSeaAmount = strAnonymizeSeaAmount.remove(strAnonymizeSeaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSeaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSeaAmount*COIN) nMaxToAnonymize = nAnonymizeSeaAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSeaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSeaAmount));
        strAnonymizeSeaAmount = strAnonymizeSeaAmount.remove(strAnonymizeSeaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSeaAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
